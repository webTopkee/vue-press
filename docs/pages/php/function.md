...
在一个函数(A 函数)中，通过变量函数的的方式$var()调用另外一个函数(B函数，C函数)，而其函数的名称通过A函数的参数传递进来，这种方式就是回到函数。因为函数A中的变量函数$var() 会根据用户传入不同的实参回过头调用不同名称的其他函数。
...
在函数中，实参实际传入个数允许超过形参的个数，在此情况下如果想获取所有传入的实参，可以使用函数 func_get_args()来获取所有实参组成的数组，在函数内部进行其它操作。
...
fn(1,2,3,4,5); //array(5) { [0]=> int(1) [1]=> int(2) [2]=> int(3) [3]=> int(4) [4]=> int(5) }
```
